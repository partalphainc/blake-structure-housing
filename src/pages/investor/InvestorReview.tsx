...
import { Building2, BarChart3, FileText, Users, LayoutDashboard, MessageSquare, ClipboardList, Star } from "lucide-react";
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <button key={i} type="button" onMouseEnter={() => setHovered(i + 1)} onMouseLeave={() => setHovered(0)} onClick={() => setRating(i + 1)} className="transition-transform hover:scale-110">
                      <Star className={`w-8 h-8 transition-colors ${i < (hovered || rating) ? "text-yellow-400 fill-yellow-400" : "text-muted-foreground"}`} />
                    </button>
                  ))}
                </div>
              </div>
              <div className="space-y-2"><Label>Display Name (optional)</Label><Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Jane S." /></div>
              <div className="space-y-2"><Label>Review Title</Label><Input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Excellent property management team!" /></div>
              <div className="space-y-2"><Label>Your Review *</Label><Textarea value={body} onChange={e => setBody(e.target.value)} placeholder="Tell us about your experience as an investor..." rows={5} /></div>
              <p className="text-xs text-muted-foreground">Reviews are reviewed by management before being published.</p>
              <Button variant="cta" className="w-full" onClick={() => submitReview.mutate()} disabled={!body || !rating || submitReview.isPending}>
                Submit Review
              </Button>
            </CardContent>
          </Card>
